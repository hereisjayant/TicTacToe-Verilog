...
namespace eval ::de1 {
    ...
    image create photo tic_tac_toe_O -data ""
    image create photo tic_tac_toe_X -data ""
    ...
    proc sw_init { y_base } {
        ...
        for { set id 0 } { $id < 10 } { incr id } {
            ...
            .de1.cv create image [expr 36 + (9 - $id) * 28] [expr 138+$y_base] -image de1_sw_0 -tags "de1_sw_$id"
    ...
    proc key_init { y_offset } {
        ...
        for { set id 0 } { $id < 4 } { incr id } {
            ...
            .de1.cv create image [expr 335 + (3 - $id) * 52] [expr 140 + $y_offset] -image de1_key_$var::key($id) -tags "de1_key_$id"
    ...
    proc led_init { y_offset } {
        for { set id 0 } { $id < 10 } { incr id } {
            array set var::led "$id z"
            .de1.cv create image [expr 36 + (9 - $id) * 28] [expr 99 + $y_offset] -image de1_led_v_z -tags "de1_led_$id"
            ::tooltip::tooltip .de1.cv -items "de1_led_${id}" "LEDR${id}"
        }
    }
    ...
    proc hex_init { y_offset } {
        for { set id 0 } { $id < 6 } { incr id } {
            for { set seg 0 } { $seg < 7 } { incr seg } {
                array set var::hex "$id,$seg 0"
            }
            set pos [expr 5 - $id]
            .de1.cv create image [expr 43 + $pos * 30 + $pos / 2 * 5] [expr 45 + $y_offset] -image de1_hex -tags "de1_hex_${id}"
            ::tooltip::tooltip .de1.cv -items "de1_hex_${id}" "HEX${id}"
            for { set seg 0 } { $seg < 7 } { incr seg } {
                .de1.cv create image [expr 43 + $pos * 30 + $pos / 2 * 5] [expr 45 + $y_offset] -image de1_hex_$seg -tags "de1_hex_${id}_${seg}" -state hidden
                ::tooltip::tooltip .de1.cv -items "de1_hex_${id}_${seg}" "HEX${id}"
            }
        }
    }
    ...
    proc play_init { y_offset } {
        .de1.cv create image 481 [expr 40 + $y_offset] -image de1_play_0 -tags "de1_play"
    ...
    proc tic_tac_toe_init { } {
        canvas .de1.cv.board -width 400 -height 400 -background white 
        place .de1.cv.board -x 75 -y 20
        
        frame .de1.cv.board.v1 -background black -height 360 -width 5
        place .de1.cv.board.v1 -x [expr 20 + 120] -y 20
        
        frame .de1.cv.board.v2 -background black -height 360 -width 5
        place .de1.cv.board.v2 -x [expr 20 + 2*120] -y 20
        
        frame .de1.cv.board.h1 -background black -height 5 -width 360
        place .de1.cv.board.h1 -x 20 -y [expr 20 + 120]
        
        frame .de1.cv.board.h2 -background black -height 5 -width 360
        place .de1.cv.board.h2 -x 20 -y [expr 20 + 2 * 120]


        for { set x_pos 0 } { $x_pos < 3 } { incr x_pos } {
            for { set y_pos 0 } { $y_pos < 3 } { incr y_pos } {
              set index [expr 8 - ($y_pos * 3 + $x_pos)]
              .de1.cv.board create image [expr 20 + 60 + ($x_pos * 120) ] [expr 20 + 60 + ($y_pos * 120) ] -image tic_tac_toe_X -tags "ttt_X_${index}" -state hidden
              .de1.cv.board create image [expr 20 + 60 + ($x_pos * 120) ] [expr 20 + 60 + ($y_pos * 120) ] -image tic_tac_toe_O -tags "ttt_O_${index}" -state hidden
            }
        }

        # horizontal win lines
        for { set y_pos 0 } { $y_pos < 3 } { incr y_pos } {
            set index [expr 2 - $y_pos]
            set y [expr 80 + (2-$y_pos) * 120] 
            .de1.cv.board create line 20 $y 380 $y -fill blue -width 5 -tag "win_${index}"
            .de1.cv.board itemconfigure win_${index} -state hidden
        }

        # vertical win lines
        for { set x_pos 0 } { $x_pos < 3 } { incr x_pos } {
            set index [expr 3 + $x_pos]
            set x [expr 80 + $x_pos * 120] 
            .de1.cv.board create line $x 20 $x 380 -fill blue -width 5 -tag "win_${index}"
            .de1.cv.board itemconfigure win_${index} -state hidden
        }

        # diagonal down
        .de1.cv.board create line 20 20 380 380 -fill blue -width 5 -tag "win_6"
        .de1.cv.board itemconfigure win_6 -state hidden

        # diagonal up
        .de1.cv.board create line 20 380 380 20 -fill blue -width 5 -tag "win_7"
        .de1.cv.board itemconfigure win_7 -state hidden
    }

    proc ttt_set_x { pos value } {
        .de1.cv.board itemconfigure ttt_X_${pos} -state [if {$value==1} {lindex normal} {lindex hidden}]
    }

    proc ttt_set_o { pos value } {
        .de1.cv.board itemconfigure ttt_O_${pos} -state [if {$value==1} {lindex normal} {lindex hidden}]
    }

    proc ttt_set_win { pos value } {
        .de1.cv.board itemconfigure win_${pos} -state [if {$value==1} {lindex normal} {lindex hidden}]
    }

    proc init {} {
        if ({[winfo exists .de1]}) { destroy .de1 }
        toplevel .de1 -width 550 -height 600 
        wm minsize .de1 550 600
        wm maxsize .de1 550 600
        wm title .de1 "Lab 3 Simulated DE1-SoC and VGA Interface"
        canvas .de1.cv -width 550 -height 600 -background #0f1d1b

        pack .de1.cv

        set y_offset [expr 600 - 173]
        sw_init $y_offset
        key_init $y_offset
        led_init $y_offset
        hex_init $y_offset
